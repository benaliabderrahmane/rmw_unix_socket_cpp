...
//
// A listener callback runs on this thread, so it must not block and must not
// destroy its own endpoint (listener_unwatch would deadlock against the drain
// it is called from). rclcpp's callbacks only push to a queue, which is what
// the contract in rmw/event_callback_type.h expects.
...
// Stop watching `fd`. Blocks until an in-flight drain of that endpoint has
// returned, so the caller may destroy the endpoint afterwards. A no-op for an
// fd that was never watched.

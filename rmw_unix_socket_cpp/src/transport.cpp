  ...
  const char * peer_label)
{
  ...
  switch (err) {
    ...
    case ENOBUFS:
      ...
      // the dropped datagram's size, not the buffer's fill level - and for a
      // payload routed through shm the datagram is only its descriptor.
      RMW_UDS_LOG_WARN_THROTTLE(
        1000,
        "UDS send to %s (socket '%s') dropped: subscriber recv queue full — "
        "dropped a %zu-byte datagram (errno=%s). "

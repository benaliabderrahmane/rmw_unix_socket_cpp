...
Everything in this section applies to the inline path: topic payloads under 64 KiB, TRANSIENT_LOCAL topics, and all service traffic. Topic payloads at or above 64 KiB bypass the datagram size cap entirely (see Large payloads below).
...
This mostly stops mattering for topics because point clouds, images, and other large payloads ride the shared-memory ring instead of the datagram. The cap still bites TRANSIENT_LOCAL topics and large service payloads, which always send inline. To raise it, increase both sysctls (the send side is bounded by `wmem_max`, the receive side by `rmem_max`, so both must be raised) before the nodes start:

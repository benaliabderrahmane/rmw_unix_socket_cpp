...
Everything in this section applies to the inline path: topic payloads under 64 KiB, small TRANSIENT_LOCAL payloads, large TRANSIENT_LOCAL payloads published through `rmw_publish_serialized_message`, and all service traffic. Topic payloads at or above 64 KiB — including large TRANSIENT_LOCAL payloads published through `rmw_publish` — bypass the datagram size cap by going through shared memory (see Large payloads below).
...
This mostly stops mattering for topics because point clouds, images, and other large payloads ride shared memory instead of the datagram — including large TRANSIENT_LOCAL topics, whose latched payloads are staged into a durable per-cache-entry segment (see Large payloads below). The cap still bites large service payloads, large TRANSIENT_LOCAL payloads published through `rmw_publish_serialized_message` (which does not feed the replay cache), and the shm-failure inline fallback — all of which still send inline. To raise it, increase both sysctls (the send side is bounded by `wmem_max`, the receive side by `rmem_max`, so both must be raised) before the nodes start:

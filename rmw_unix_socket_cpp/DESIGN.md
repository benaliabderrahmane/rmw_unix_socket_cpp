...
`recv_from` returns one parsed message at a time. The subscription drains its socket in a loop: `drain_endpoint` calls `recv_from` repeatedly until it returns false (socket empty). Each datagram is filtered by `msg_type` so that only topic messages land in this queue, wrapped in a `ReceivedMessage` (header plus payload), and pushed onto the per-subscription queue under its mutex. The queue is capped at the QoS `depth`; when a push exceeds the cap the oldest entry is dropped, which is the KEEP_LAST history policy enforced at the rcl boundary rather than in the kernel. This drain runs inside `rmw_wait`, not on a background thread, which is why there is no receiver thread to manage.

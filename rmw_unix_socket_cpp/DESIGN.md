  ...
  +-----------------------------+
        ...
        v
                                          ...
                                          +---------------------------+
                                                      ...
                                                      v
                                          +---------------------------+
                                          | recv_from: probe size     |
                                          | (zero-copy peek), scatter |
                                          | read header + payload     |
                                          +---------------------------+
                                                      |
                                                      v
...
- `payload_size` (4 bytes): the CDR payload length. The receiver cross-checks it against the bytes that actually arrived to reject corrupt or foreign datagrams (see Validating the read).
...
Reading one datagram off a socket is done by `recv_from` in `transport.cpp`. The complication it solves is that the caller does not know in advance how big the next message is, and a `SOCK_DGRAM` `recv` that supplies too small a buffer silently truncates the datagram and discards the rest. The function avoids that in two steps, and reads the payload out of the kernel exactly once:

1. **Probe the size without copying.** A first `recv` is issued with a null, zero-length buffer and `MSG_PEEK | MSG_TRUNC | MSG_DONTWAIT`. `MSG_PEEK` leaves the datagram on the socket, and `MSG_TRUNC` makes the call return the datagram's true length even though the supplied buffer is empty — so the probe learns the size while moving zero payload bytes. The return value `n` is the real size on the wire.
2. **Scatter-read straight into the destination.** The caller's `payload_out` vector is resized to `n - sizeof(WireHeader)`, and a single `recvmsg` with a two-entry `iovec` consumes the datagram: the first entry lands the wire header directly in `header_out`, the second lands the payload directly in `payload_out` (header-only datagrams use a one-entry `iovec`, mirroring the send path). This mirrors the gather-write on the send path (`send_to`'s `sendmsg` with header + payload `iovec`), and it is the only copy the payload makes between the kernel and the queue entry.

An earlier version of `recv_from` peeked into a thread-local scratch buffer, `recv`'d into it again, and then copied the payload a third time into `payload_out`; for a 4 MB message that moved ~12 MB through userspace per delivery. The probe/scatter form moves 4 MB, and also drops the scratch buffer itself, which grew to the largest message ever seen on the thread and never shrank. One behavioral fix rode along: the old code returned without consuming a zero-length datagram, so a rogue empty datagram sat at the head of the FIFO forever, wedging the socket; runts including the zero-length case are now consumed and dropped.

`recv_from` distinguishes the outcomes of the probe. A negative return with `errno == EAGAIN`/`EWOULDBLOCK` is the steady-state "nothing to read" case driven by the `rmw_wait` loop and is silent by design; any other negative errno (for example `EBADF` after shutdown) is logged throttled. A return shorter than `sizeof(WireHeader)` — including a zero-length datagram — is a runt that cannot be one of ours: it is consumed with a zero-length read and dropped (logged throttled), because a merely-peeked runt would sit at the head of the FIFO forever, blocking real messages behind it and waking the level-triggered epoll on every wait.

**Validating the read.** Each datagram is one `WireHeader` (the 37-byte struct built on the send path) immediately followed by the serialized message body. After the `recvmsg`, `recv_from` validates:

- If the kernel set `MSG_TRUNC` in `msg_flags`, the datagram read was bigger than the buffer sized from the probe — possible only when another thread consumed the probed datagram in between. The truncated message is unusable and is dropped (logged throttled).
- The payload length `n - sizeof(WireHeader)` is cross-checked against the `payload_size` field the sender wrote into the header. A mismatch means a corrupt or foreign datagram; the length is clamped so the caller never sees more bytes than the wire really carried, and the event is logged.
- `payload_out` is trimmed to the validated length — a no-op in the common case.
...
There are no background threads, so all sends, drains, and takes run on the caller's thread, typically the executor thread inside `rmw_wait`. Under a `MultiThreadedExecutor`, several threads may call `rmw_take`, `rmw_publish`, and `rmw_wait` concurrently on different entities. The data structures are sized for this: each subscription, service, and client serializes socket reads with a per-entity `recv_mutex` (held for the whole drain loop, because `recv_from`'s size probe and consuming read are two syscalls that must not interleave with another drain of the same fd) and guards its own receive queue with a per-entity `queue_mutex`, and each publisher, service, and client guards its discovery cache with its own cache mutex (`sub_cache_mutex`, `cache_mutex`, `svc_cache_mutex`, `client_cache_mutex`). Concurrent operations on different entities therefore do not contend. The one process-wide shared resource on the send side is the single send socket; concurrent senders on it are safe because the kernel delivers each `SOCK_DGRAM` `sendmsg` as one atomic datagram, so two threads' messages never interleave.

...
**Staging and fanout.** Staging is one `memcpy` into the ring under the record's seqlock: the record's `seq` is set odd (an absolute `exchange`, because the slot may hold stale bytes), the payload is copied, and `seq` is set to the even value derived from the publisher's monotonic record counter. The descriptor — segment identity, record offset, expected `seq`, payload length — is then fanned out with the normal `sendmsg` loop. N subscribers cost one payload copy plus N tiny datagrams, instead of N kernel copies of the payload.
...
**Copy accounting.** The inline path moves a payload through the kernel twice per subscriber — 2N copies for N subscribers. The ring path costs one copy into the ring plus one out per interested subscriber — 1+N copies — with no syscall proportional to the payload and no dependency on `net.core.wmem_max`.

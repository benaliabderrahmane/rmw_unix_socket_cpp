...
**A watched endpoint may still be in a wait set, and `delivery_fd` is what makes that safe.** Both the listener and `rmw_wait` may drain the same socket; that much is fine, since both use non-blocking `recv` into the same mutex-protected queue. The hazard is a lost wakeup. A wait scans its queues, finds them empty, and then calls `epoll_wait`; if the listener drains the socket in that gap, the socket is no longer readable, `epoll` has nothing left to report, and the wait sleeps through a message that is already sitting in the queue.

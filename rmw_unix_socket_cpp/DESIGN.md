...
The first is **reordering**. `drain_endpoint` `recv`s outside `queue_mutex` and pushes inside it, so two concurrent drains can interleave as recv A / recv B / push B / push A and leave the queue holding one publisher's samples out of order. The queue mutex prevents corruption, not reordering, and ROS 2 guarantees per-publisher FIFO. A per-endpoint `drain_mutex`, held for the whole drain, serialises them — it is taken before `queue_mutex` and `callback_mutex`, and nothing takes `listener_mutex` while holding it, so the lock graph stays acyclic. The cost is that a drain in flight briefly blocks another drain of the *same* endpoint; different endpoints never contend.
...
The rule this implies is **context-wide, not endpoint-local**, because there is one `listener_mutex` per context rather than one per endpoint: a listener callback must not destroy, register a callback on, or clear the callback of **any** endpoint in the context, and must not create or destroy a wait set either, since that mutex also guards the delivery-fd list. Each of those would wait on a mutex its own caller holds. It must not call `rmw_take` on its own endpoint either — the callback fires inside `drain_endpoint`, which holds that endpoint's `drain_mutex`. For the same reason `listener_unwatch` blocks on whatever drain is in flight, not specifically on a drain of the endpoint being unwatched. `rclcpp`'s callbacks only push to a queue, which is what the contract in `rmw/event_callback_type.h` expects of them.

...
The fix is the doorbell's ordering pair, reused. The listener signals a per-context `delivery_fd` eventfd **strictly after** it enqueues, and `rmw_wait` drains `delivery_fd` **strictly before** it scans its queues. A message therefore either lands in that scan or leaves the level-triggered eventfd readable for the `epoll_wait` that follows — there is no order in which it is missed. `delivery_fd` is armed only while the listener runs, so a process that never registers a callback never sees it. Because it is signalled for any endpoint in the context, not just the ones a given wait set holds, a wake on it is not treated as progress: the dispatch re-runs the caller's own queue scan and ends the wait only if something the caller actually waits on became ready.

**Lifetime.** `listener_mutex` is held across the drain and the callback it fires, and `listener_unwatch` takes the same mutex, so a thread destroying an endpoint blocks until any in-flight drain of it has returned. `rmw_destroy_subscription`, `rmw_destroy_service` and `rmw_destroy_client` unwatch before they free anything. The rule this implies: **a listener callback must not destroy its own endpoint**, because the `listener_unwatch` inside that destroy would wait on a mutex its own caller holds. `rclcpp`'s callbacks only push to a queue, which is what the contract in `rmw/event_callback_type.h` expects of them.

**What it is not.** The listener is edge-driven: it wakes on socket readiness and never on a clock. It is not a timer, so it does not make deadline or liveliness QoS enforceable, and it generates no QoS status events (see Timing policies: accepted, not enforced). Adding those would need a periodic wakeup, which is a different mechanism and a different decision.

...
**When it runs.** There are three triggers.
...
The third is periodic and runs from the graph introspection path in `rmw_graph.cpp`. Without it a process that never restarts and never fills the table would keep serving a crashed node's endpoints forever, because the other two triggers only fire at startup and at exhaustion. It is rate-limited to once per second per context (`GRAPH_CLEANUP_INTERVAL_NS`) rather than running on every query: the sweep costs one `stat("/proc/<pid>")` per live entity, and graph introspection is polled — `topic_tools` discovery runs at 10 Hz per node — so an unconditional sweep would turn every poll into a syscall storm proportional to the size of the whole graph. The interval is the only thing it bounds: how long a crashed node's slots stay visible, not whether they are reclaimed.

The sweep also de-duplicates by PID. A process owns a node slot plus a slot for every endpoint under it, so checking liveness once per distinct PID rather than once per slot is the difference between a syscall per entity and a syscall per process.

**The scan bound recovers.** `high_water_slot` is widened by adds, and the cleanup pass narrows it again to the highest still-occupied index it saw. Without that, a supervisor that restarts nodes would ratchet the bound up permanently and every later scan would pay for slots that drained long ago. Narrowing is safe against a concurrent add because `try_add_once` widens the bound with a monotonic CAS *after* publishing its slot: an add that already widened past the sweep makes the narrowing CAS fail, and one that has not yet widened will do so and restore the bound itself.

### Scaling characteristics

A discovery scan (`registry_query`) and the stale-cleanup pass (`registry_cleanup_stale`) loop over `[0, high_water_slot)`, the used prefix of the table, rather than all `REGISTRY_MAX_ENTRIES` (32768) slots; adds widen that bound and cleanup narrows it back as the tail drains. A worst-case add (`try_add_once`) still scans the full table, since it has to find an empty slot anywhere. The per-empty-slot work inside the prefix is a single atomic load, so a sparsely-populated table is still scanned quickly. `registry_query` also compares the topic name before taking a snapshot, so filtering by topic skips the 1.1 kB per-slot copy for entries that cannot match.
...
Graph introspection uses the same guard. `rmw_graph.cpp` caches each distinct query against the generation it was taken at, so a repeated poll of an unchanged graph — which is what discovery in `topic_tools` and `rosbag2` does — costs one atomic load instead of a scan.
...
Graph events and `TRANSIENT_LOCAL` late-joiner replay are serviced from inside `rmw_wait` (woken by the doorbell). A process none of whose threads ever enters `rmw_wait` — an executor wedged in a user callback, or a bare-rmw publisher that never waits — cannot replay its latched messages or observe graph changes until it next waits or publishes. Standard rclcpp nodes always have a waiting thread (the GraphListener), so this bites only unusual bare-rmw setups.

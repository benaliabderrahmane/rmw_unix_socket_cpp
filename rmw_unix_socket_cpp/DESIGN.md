...
Everything in this section applies to the inline path: topic payloads under 64 KiB, small TRANSIENT_LOCAL payloads, and all service traffic. Topic payloads at or above 64 KiB — including large TRANSIENT_LOCAL payloads, whether published through `rmw_publish` or `rmw_publish_serialized_message` — bypass the datagram size cap by going through shared memory (see Large payloads below).
